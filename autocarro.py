...
#         queue_sizes.append(len(passenger_queue[stop_id]))
#         next_passenger_id += 1
#         next_passenger_arrival = current_time + max(1, int(np.random.exponential(MEAN_PASSENGER_ARRIVAL)))
#         if next_passenger_arrival < SIMULATION_TIME:
#             schedule_event(next_passenger_arrival, 'passenger_arrival')
...
#         if stop_id < NUM_BUS_STOPS - 1:
#             next_stop_time = current_time + max(1, int(np.random.normal(MEAN_STOP_INTERVAL, STD_STOP_INTERVAL)))
#             schedule_event(next_stop_time, 'bus_arrival', entity_id=entity_id, stop_id=stop_id + 1)
...
# STD_PASSENGER_ARRIVAL = float(input("Enter the standard deviation for passenger arrival interval in minutes: "))
# BUS_CAPACITY = int(input("Enter the bus capacity: "))
# MEAN_LATE_EARLY = float(input("Enter the mean deviation for bus arrival (negative for early, positive for late) in minutes: "))
...
for i in range(start_time, end_time, BUS_INTERVAL):
    schedule_event(i, 'bus_arrival', entity_id=(
i - start_time) // BUS_INTERVAL + 1, stop_id=0)
schedule_event(next_passenger_arrival, 'passenger_arrival')
